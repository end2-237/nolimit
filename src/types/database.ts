 ...
 * - Mouvements en attente d'approbation (entrées ET sorties)
 * - pending_in / pending_out : soumis par opérateur, validé par admin
...
const DB_VERSION = 5; // Bumped for pending_out support
...
class DatabaseService {
  ...
  async loadDemoData(): Promise<void> {
    ...
    const products: Product[] = demoProducts.map((p, i) => {
      ...
      return {
        ...
        name: p.name, sku, category: p.category, sub_type: p.sub_type,
        description: p.description, unit: p.unit, price: p.price, threshold: p.threshold,
        expiry_date: p.expiry_date, image_url: null,
        created_at: this.now(), updated_at: this.now(), count: 0,
      } as Product;
    });
    ...
    products.forEach((product, i) => {
      APP_CONFIG.sites.forEach((site, si) => {
        stocks.push({
          ...
          last_delivery: `2026-04-0${si + 5}`, updated_at: this.now(),
        });
      });
    });
    ...
    const demoUsers = [
      ...
      { username: 'ngono', password: 'ngono123', full_name: 'Ngono Patricia', email: 'ngono@nolimit.cm', role: 'operator' as const, site_ids: JSON.stringify(['YDE']) },
    ];
  ...
  createProduct(data: Omit<Product, 'id' | 'created_at' | 'updated_at'>): Product {
    ...
    APP_CONFIG.sites.forEach(site => {
      const stock: Stock = {
        ...
        product_id: product.id, site_id: site.id,
        quantity: 0, last_delivery: null, updated_at: this.now(),
      };
   ...
   *
   * RÈGLE PRINCIPALE :
   * - Admin/Manager : peut confirmer directement OU soumettre en pending
   * - Operator/Viewer : TOUJOURS pending, jamais confirmed
   * - pending_in : entrée en attente (stock non modifié)
   * - pending_out : sortie/vente en attente (stock non modifié)
   * - Le stock ne bouge QUE lors de l'approbation (approveMovement)
   */
  createMovement(data: Omit<Movement, 'id' | 'created_at'>): Movement | { error: string } {
    // For direct confirmed out/transfer/damage — check stock immediately
    ...
    // Only update stocks for directly confirmed movements (admin bypass)
    if (data.status === 'confirmed') {
      this._applyMovementToStock(movement);
    } else if (data.status === 'pending') {
      // Create pending_approval alert
      ...
      if (product) {
        ...
        const siteName = APP_CONFIG.sites.find(s => s.id === (data.from_site_id || data.to_site_id))?.name || '';
        const alert: Alert = {
  ...
  approveMovement(movementId: number, approverId: number): boolean {
    ...
    // Check stock for out movements before approving
    if (m.type === 'pending_out' || m.type === 'out') {
      const fromStock = this.cache.stocks.find(s => s.product_id === m.product_id && s.site_id === m.from_site_id);
      if (!fromStock || fromStock.quantity < m.quantity) {
        // Reject automatically if stock insufficient
        m.status = 'rejected';
        m.approved_by = approverId;
        m.approved_at = this.now();
        m.rejection_reason = `Stock insuffisant au moment de la validation: ${fromStock?.quantity || 0} disponible(s)`;
        idb.put('movements', m);
        return false;
      }
    }
    ...
    // Convert pending types to their final types
    ...
    // Remove the pending alert for this product
    const alertIdx = this.cache.alerts.findIndex(
      a => a.type === 'pending_approval' && a.product_id === m.product_id
    );
    if (alertIdx !== -1) {
  ...
  private _applyMovementToStock(m: Movement) {
    if ((m.type === 'in') && m.to_site_id) {
      ...
      if (m.from_site_id) {
        ...
        if (fi !== -1) { this.cache.stocks[fi].quantity = Math.max(0, this.cache.stocks[fi].quantity - m.quantity); idb.put('stocks', this.cache.stocks[fi]); }
      }
      if (m.to_site_id) {
  ...
  getDashboardStats(allowedSites?: string[]) {
    ...
    const todayMovements = this.cache.movements.filter(m => m.created_at.startsWith(todayStr) && m.status === 'confirmed').length;
