...
export function OutboxPanel({ onClose }: OutboxPanelProps) {
  ...
  return (
    ...
    >
      ...
      <div className="max-h-[380px] overflow-y-auto divide-y divide-gray-100">
        {items.length === 0 ? (
          <p className="py-8 text-center text-sm text-gray-400">File vide</p>
        ) : (
          items.map(item => {
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-gray-400 mt-0.5">
                      {new Date(item.createdAt).toLocaleString('fr-FR', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' })}
                  ...
                  <div className="flex flex-col gap-1 flex-shrink-0">
                    <button
                      ...
                      disabled={isRetrying}
                      title="Réessayer"
                      className="w-7 h-7 flex items-center justify-center rounded-lg text-blue-500 hover:bg-blue-50 disabled:opacity-40"
                    >
      ...
      {items.length > 0 && (
        <div className="px-4 py-2 bg-orange-50 border-t border-orange-100 text-[11px] text-orange-600">
          Envoi automatique à la reconnexion · Cliquez 🔄 pour forcer
        </div>
      )}
    </div>
  );
