          ...
          <Reveal>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: 'var(--sans)', fontSize: 13, letterSpacing: '0.05em', color: 'var(--muted)', marginTop: 6 }}>{t.age} ans · {t.soin}</div>
                <div style={{ display: 'flex', gap: 4, marginTop: 14, color: 'var(--gold)' }}>
                  {Array.from({ length: 5 }).map((_, j) => <span key={j} style={{ opacity: j < t.stars ? 1 : 0.2 }}>★</span>)}
                </div>
              </div>
            </div>
          </Reveal>
            ...
            <div className="marquee-track">
              {[...PRESS, ...PRESS].map((p, j) => (
                <span key={j} style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 'clamp(22px, 2.4vw, 36px)', color: 'rgba(26,26,26,0.55)', fontWeight: 300, whiteSpace: 'nowrap', letterSpacing: '0.02em' }}>
                  {p}<span style={{ marginLeft: 80, color: 'var(--terracotta)', fontSize: '0.8em' }}>✦</span>
                </span>
              ))}
            </div>
          </div>
